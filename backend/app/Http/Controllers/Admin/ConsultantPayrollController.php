...
class ConsultantPayrollController extends Controller
{
    ...
    public function generateDraft(Request $request): JsonResponse
    {
        ...
        abort_if($employeeCount < 1, 422, 'No active Consultant employees in the selected scope.');

        $batchKey = 'consultant_'.$this->makeBatchKey($companyId, $branchId, $departmentId, $employeeIds[0] ?? null, $from, $to, $validated['pay_cycle_id'] ?? null);
        ...
        if ($conflictingFinalized instanceof PayrollBatchRun) {
            abort(
                422,
                'This Consultant payroll period is already finalized (batch #'.$conflictingFinalized->id.'). Void that batch before generating a new draft.'
            );
        }
        ...
        if ($existing instanceof PayrollBatchRun && (string) $existing->status === PayrollBatchRun::STATUS_FINALIZED) {
            abort(422, 'This Consultant payroll period is already finalized. Void the finalized batch before generating a new draft.');
        }
        if ($existing instanceof PayrollBatchRun
            ...
            return response()->json([
                'message' => 'Consultant payroll draft is already processing.',
                'queued' => true,
        ...
        return response()->json([
            'message' => 'Consultant payroll draft generation queued.',
            'queued' => true,
    ...
    {
        $run = $this->consultantRun($batchRunId);
        abort_if((string) $run->status === PayrollBatchRun::STATUS_FINALIZED, 422, 'Cannot recompute finalized Consultant payroll.');

        Payslip::query()
            ->where('payroll_batch_run_id', (int) $run->id)
            ->where('payroll_module', PayrollBatchRun::MODULE_CONSULTANT)
        ...
        return response()->json([
            'message' => 'Consultant payroll draft recompute queued.',
            'payroll_batch_run_id' => (int) $run->id,
            'status' => PayrollBatchRun::STATUS_QUEUED,
        ], 202);
    }
    ...
    {
        $run = $this->consultantRun($batchRunId);
        abort_if((string) $run->status === PayrollBatchRun::STATUS_FINALIZED, 422, 'Consultant payroll is already finalized.');
        abort_unless(
            in_array((string) $run->status, [PayrollBatchRun::STATUS_DRAFT, PayrollBatchRun::STATUS_FAILED], true),
            422,
            'Only draft or failed Consultant payroll can be finalized.'
        );
    ...
    public function recentBatches(Request $request): JsonResponse
    {
        ...
        $paginator->getCollection()->transform(function (PayrollBatchRun $run): array {
            ...
            return [
                ...
                'payroll_batch_run_id' => (int) $run->id,
                'payroll_module' => PayrollBatchRun::MODULE_CONSULTANT,
                'module_label' => 'Consultant Payroll',
    ...
    public function downloadReportExcel(Request $request, int $batchRunId): mixed
    {
        return $this->downloadReportExcelFile($request, $batchRunId);
    }

    public function downloadDeductionsReportExcel(Request $request, int $batchRunId): mixed
    {
        return $this->downloadReportExcelFile($request, $batchRunId, true);
    }

    private function downloadReportExcelFile(Request $request, int $batchRunId, bool $deductionOnly = false): mixed
    {
        $run = $this->consultantRun($batchRunId);
        abort_if((string) $run->status !== PayrollBatchRun::STATUS_FINALIZED, 422, 'Payroll report is only available for finalized Consultant batches.');

        $actor = $request->user();
        abort_unless($actor instanceof User, 403);

        try {
            $result = $this->payrollReportService->excelForRun($run, $actor, $deductionOnly);
        } catch (\RuntimeException $e) {
            abort(422, $e->getMessage());
        }

        return response()->streamDownload($result['write'], 'consultant_'.$result['filename'], [
    ...
    private function downloadReportPdf(Request $request, int $batchRunId, bool $deductionOnly = false): mixed
    {
        ...
        return $result['pdf']->download('consultant_'.$result['filename']);
    }
    ...
    private function payslipList(Request $request, array|string $status): JsonResponse
    {
        ...
        $query = Payslip::query()
            ->with([
                'employee:id,name,first_name,last_name,employee_code,department,department_id,company_id',
    ...
    private function paginatedPayslipListResponse(\Illuminate\Database\Eloquent\Builder $query, ?int $perPage = null): JsonResponse
    {
        ...
        $paginator->getCollection()->transform(function (Payslip $payslip): array {
            ...
            // Keep list amounts on the persisted payslip columns for Consultant so batch MetricCards
            // and row values do not diverge when sanitize only reshapes display lines.
            if ((string) ($payslip->payroll_module ?? '') === PayrollBatchRun::MODULE_CONSULTANT) {
    ...
    private function consultantRun(int $batchRunId): PayrollBatchRun
    {
