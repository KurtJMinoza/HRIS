...
class EmployeeCertificationController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $items = EmployeeCertification::where('user_id', $user->id)
    ...
    public function update(Request $request, int $id): JsonResponse
    {
        /** @var User $user */
        $user = $request->user();
        $cert = EmployeeCertification::where('id', $id)->where('user_id', $user->id)->firstOrFail();

        if ($cert->verification_status === 'verified') {
            throw ValidationException::withMessages(['certification' => ['Verified certifications cannot be edited.']]);
        }
        ...
        // Any employee edit re-submits for verification.
