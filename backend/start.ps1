...
function Start-QueueWorker([string] $queue, [int] $timeout, [int] $count) {
  for ($i = 1; $i -le $count; $i++) {
    Start-Process php -ArgumentList "artisan queue:work redis --queue=$queue --timeout=$timeout --sleep=1 --tries=1" -WorkingDirectory $backendPath -WindowStyle Hidden | Out-Null
  }
}
