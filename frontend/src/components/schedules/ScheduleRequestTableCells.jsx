 ...
 */
function scheduleRequestStatusKey(item) {
...
export function ScheduleRequestStatusBadge({ item }) {
  ...
  const currentStep = Array.isArray(item?.approval_progress)
    ? item.approval_progress.find((step) => step?.status === 'current')
    : null
  const currentStage = normalizeApprovalHeadTitle(item?.current_stage || currentStep?.label)
  const label = item?.status === 'pending' && currentStage
    ? `Waiting for ${currentStage}`
    : normalizeApprovalStatusLabel(
