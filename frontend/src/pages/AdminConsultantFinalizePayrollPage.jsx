...
import {
  ...
  adminFinalizeConsultantPayroll,
  adminPollAndDownloadBulkPayslipZip,
  adminQueueBulkPayslipDownload,
  adminRecomputeConsultantPayroll,
  getConsultantPayrollBatchStatus,
  getConsultantPayrollBatchPayslips,
  getConsultantPayrollBatches,
  getConsultantPayrollPayslips,
...
function rowMonthlySalary(row) {
  ...
  return Number(
    summary.fixed_salary
      ...
      ?? summary.Consultant_fixed_salary
...
function earningLines(row) {
  ...
  return basic > 0 ? [{ key: 'Consultant_basic_pay_fallback', label: 'Basic Pay', amount: basic }] : []
}
...
export default function AdminConsultantFinalizePayrollPage() {
  ...
  const [deductionsExcelDownloading, setDeductionsExcelDownloading] = useState(false)
  ...
  useEffect(() => {
    ...
    if (draftFailed) {
      ...
      toast({
        title: 'Consultant payroll draft generation failed',
    ...
    draftToastShownRef.current = true
    toast({
      title: 'Consultant payroll draft generated successfully.',
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(async () => {
      ...
      try {
        const status = await getConsultantPayrollBatchStatus(selectedId)
        setSelectedBatch(status)
        const nextStatus = String(status?.status || '').toLowerCase()
        if (nextStatus === 'draft' || nextStatus === 'failed') {
          const list = await getConsultantPayrollBatchPayslips(selectedId, { per_page: 100 })
  ...
  async function loadBatches() {
    const data = await getConsultantPayrollBatches({ per_page: 50 })
  ...
  async function loadSelected(batchId = selectedId) {
    ...
    setLoading(true)
    try {
      const status = await getConsultantPayrollBatchStatus(batchId)
      const list = await getConsultantPayrollBatchPayslips(batchId, { per_page: 100 })
      setSelectedBatch(status)
      const rows = Array.isArray(list.payslips?.data) ? list.payslips.data : Array.isArray(list.data) ? list.data : []
      setPayslips(rows)
    } catch (error) {
      toast({ title: 'Failed to load Consultant payroll', description: error.message, variant: 'error' })
    } finally {
      setLoading(false)
    }
  }
  ...
  useEffect(() => {
    loadBatches().catch((error) => toast({ title: 'Failed to load Consultant batches', description: error.message, variant: 'error' }))
  ...
  async function finalizeBatch() {
    ...
    try {
      const result = await adminFinalizeConsultantPayroll(selectedId)
      const runId = Number(result?.payroll_batch_run_id || selectedId)
      setQueuedRunId(runId)
      toast({ title: 'Consultant finalization queued', description: 'Finalization is running in the payroll worker.' })
  ...
  async function recomputeBatch() {
    ...
    setWorking(true)
    try {
      await adminRecomputeConsultantPayroll(selectedId)
      toast({ title: 'Consultant recompute queued' })
      await loadSelected(selectedId)
      await loadBatches()
    } catch (error) {
  ...
  async function downloadZip() {
    ...
    try {
      ...
      saveBulkPayslipZipBlob(blob, `Consultant-payslips-${selectedId}.zip`)
    } catch (error) {
  ...
  async function downloadReport() {
    if (!selectedId) return
    try {
      const blob = await getConsultantPayrollReportPdfBlob(selectedId)
      saveBlob(blob, `Consultant-payroll-report-${selectedId}.pdf`)
    } catch (error) {
  ...
  async function downloadDeductionsReport() {
    ...
    try {
      const blob = await getConsultantPayrollDeductionsPdfBlob(selectedId)
      saveBlob(blob, `Consultant-payroll-deductions-report-${selectedId}.pdf`)
    } catch (error) {
  ...
  async function downloadReportExcel() {
    if (!selectedId || reportExcelDownloading) return
    setReportExcelDownloading(true)
    try {
      const blob = await getConsultantPayrollReportXlsxBlob(selectedId)
      saveBlob(blob, `Consultant-payroll-report-${selectedId}.xlsx`)
    } catch (error) {
      toast({ title: 'Report Excel download failed', description: error.message, variant: 'error' })
    } finally {
      setReportExcelDownloading(false)
    }
  }

  async function downloadDeductionsExcel() {
    if (!selectedId || deductionsExcelDownloading) return
    setDeductionsExcelDownloading(true)
    try {
      const blob = await getConsultantPayrollDeductionsXlsxBlob(selectedId)
      saveBlob(blob, `Consultant-payroll-deductions-report-${selectedId}.xlsx`)
    } catch (error) {
      toast({ title: 'Deductions Excel download failed', description: error.message, variant: 'error' })
    } finally {
      setDeductionsExcelDownloading(false)
    }
  }
  ...
  async function bulkSendBatchPayslips() {
    ...
    try {
      ...
      await loadSelected(selectedId)
      toast({
        title: 'Consultant payslips sent',
  ...
  async function deleteOrVoidBatch() {
    ...
    try {
      if (isFinalized) {
        ...
        toast({ title: 'Consultant payroll voided', description: 'The finalized batch was voided and active locks were released.' })
      } else {
        await adminDeletePayslipBatch(selectedId)
        toast({ title: 'Consultant draft deleted', description: 'The draft batch and draft payslips were removed.' })
      }
      setSelectedBatch(null)
      setPayslips([])
      setShowDeleteModal(false)
      await loadBatches()
      navigate(`${hrBase}/compensation/generate-payslips?module=Consultant`)
    } catch (error) {
  ...
  const totals = useMemo(() => {
    ...
    const employeeCount = Number(batchTotals.employee_count ?? aggregate.payslip_count ?? selectedBatch?.employee_count ?? 0)
      || payslips.length
    // Prefer batch aggregate so MetricCards stay aligned with persisted Consultant payroll
  ...
  useEffect(() => {
    ...
    pollRef.current = setInterval(async () => {
      ...
      try {
        const status = await getConsultantPayrollBatchStatus(queuedRunId)
        ...
        if (s === 'finalized') {
          ...
          await loadBatches()
          toast({ title: 'Consultant payroll finalized', description: 'Payslips and payroll locks are now in place.' })
        ...
        if (s === 'failed') {
          ...
          toast({
            ...
            description: status?.error_message || 'Consultant payroll finalize job failed.',
            variant: 'error',
          })
          return
        }
    ...
    <div className={cn(PAGE_SHELL, PAGE_STACK)}>
      <div className="flex flex-wrap items-center justify-between gap-2">
        <Button
          ...
          onClick={() => navigate(`${hrBase}/compensation/generate-payslips?module=Consultant`)}
        >
        ...
        <div className="flex flex-wrap items-center justify-end gap-2">
          {finalized && !voided ? (
            <>
              ...
              <Button type="button" variant="outline" size="sm" className="h-10 gap-2 rounded-xl border-border/80 bg-card font-semibold shadow-sm hover:bg-muted" onClick={downloadReportExcel} disabled={loading || working || reportExcelDownloading}>
                {reportExcelDownloading ? <Loader2 className="h-4 w-4 animate-spin" /> : <FileSpreadsheet className="h-4 w-4" />}
                Payroll Report Excel
              </Button>
              <Button type="button" variant="outline" size="sm" className="h-10 gap-2 rounded-xl border-border/80 bg-card font-semibold shadow-sm hover:bg-muted" onClick={downloadDeductionsExcel} disabled={loading || working || deductionsExcelDownloading}>
                {deductionsExcelDownloading ? <Loader2 className="h-4 w-4 animate-spin" /> : <FileSpreadsheet className="h-4 w-4" />}
                Deductions Excel
              </Button>
            </>
          ) : null}
      ...
      <div className={cn(CARD, 'p-5 md:p-6')}>
        <div className="flex flex-col gap-5 @md:flex-row @md:items-start @md:justify-between">
          <div className="space-y-2">
            <Badge className="w-fit border-brand/30 bg-brand/10 text-brand hover:bg-brand/10">Consultant Payroll</Badge>
            ...
            <p className="max-w-3xl text-[15px] font-medium leading-7 text-muted-foreground">
              Review fixed Basic Pay drafts, shared deductions, statutory lines, and generated payslip rows, then finalize to lock the Consultant payroll run.
            </p>
          </div>
        ...
        <div className="mt-5 grid gap-3 @md:grid-cols-[minmax(0,1fr)_auto] @md:items-end">
          <div className="space-y-2">
            <p className="text-sm font-semibold text-muted-foreground">Consultant batch</p>
            <div className="h-11 w-full rounded-xl border border-border/80 bg-background px-3 text-sm font-semibold text-foreground shadow-sm dark:bg-input/35 flex items-center">
              {selectedId
                ? `#${selectedId} · Consultant · ${formatDate(selectedBatch?.pay_period_start)} - ${formatDate(selectedBatch?.pay_period_end)}`
                : 'No Consultant batch selected'}
            </div>
          </div>
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 text-sm @md:grid-cols-3">
          <div className="rounded-xl border border-border/80 bg-background px-4 py-3 text-muted-foreground dark:bg-input/30">
            <span className="font-semibold text-foreground">Pay Cycle:</span> {selectedBatch?.pay_cycle?.name || selectedBatch?.pay_cycle_name || 'Consultant Payroll'}
          </div>
          <div className="rounded-xl border border-border/80 bg-background px-4 py-3 text-muted-foreground dark:bg-input/30">
        ...
        {!finalized && !voided ? (
          <p className="mt-4 flex items-start gap-2 rounded-xl border border-brand/20 bg-brand/10 px-4 py-3 text-sm text-muted-foreground">
            ...
            <span>Figures below are computed for review only. PDFs are available after you confirm and finalize this Consultant payroll.</span>
          </p>
        ) : voided ? (
          <p className="mt-4 flex items-start gap-2 rounded-xl border border-red-200/70 bg-red-50 px-4 py-3 text-sm text-red-900 dark:border-red-900/40 dark:bg-red-950/30 dark:text-red-100">
            ...
            <span>
              <span className="font-semibold">This Consultant payroll batch was voided.</span> Generate a new draft for the same pay period to run payroll again.
            </span>
          </p>
        ) : finalized ? (
          <p className="mt-4 flex items-start gap-2 rounded-xl border border-brand/30 bg-brand/10 px-4 py-3 text-sm text-foreground">
            <Lock className="mt-0.5 h-4 w-4 shrink-0 text-brand" />
            <span>
              <span className="font-semibold text-brand">This Consultant payroll has been finalized and is now locked.</span> You can view payslips, bulk download PDFs, send to employees, or export the payroll report.
            </span>
          </p>
        ) : null}
      </div>
          ...
          <div className="flex flex-col gap-3 px-4 py-4 @md:flex-row @md:items-center @md:justify-between @md:px-5">
            <div>
              ...
              <p className="mt-1 text-sm font-normal leading-snug text-muted-foreground">
                Draft and finalized Consultant payslip rows for the selected batch.
              </p>
            </div>
              ...
              <Table className="min-w-[1260px] border-0 [&_td]:border-0 [&_th]:border-0">
                <TableHeader className="[&_tr]:border-0">
                  <TableRow className="border-0 bg-background hover:bg-background dark:bg-input/25 dark:hover:bg-input/25">
                    ...
                    <TableHead className="h-auto py-2.5 px-2 text-left text-[12px] font-bold uppercase tracking-normal text-muted-foreground">Company</TableHead>
                ...
                <TableBody className="[&_tr]:border-0">
                  {pageRows.map((row) => {
                    ...
                    const employeeCode = row.employee?.employee_code || row.employee_code || 'Consultant'
                    return (
                      <TableRow key={row.id} className="border-0 transition-colors hover:bg-muted/35">
                        ...
                        <TableCell className="py-3 px-2 text-sm text-muted-foreground">
                          {row.company?.name || row.employee?.company?.name || '—'}
                        </TableCell>
                        ...
                        <TableCell className="py-3 pl-2 pr-5 text-right">
                          <div className="flex flex-wrap items-center justify-end gap-2">
                            ...
                            <Button
                              ...
                              variant="outline"
                              className="h-8 rounded-lg border-border/80 bg-background font-semibold"
                              onClick={() => navigate(`${hrBase}/compensation/payslips/${row.id}/view?return_to=${encodeURIComponent(`${hrBase}/Consultant/payroll/finalize?batch_run_id=${selectedId}`)}`)}
                            >
                              View Payslip
                            </Button>
                          </div>
                  ...
                  {!loading && visiblePayslips.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={11} className="py-10 text-center text-sm text-muted-foreground">
                        {voided
                          ? 'This batch was voided. Generate a new Consultant draft for this pay period.'
                          : finalized
        ...
        <DialogContent showCloseButton={false} className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold text-foreground">
              {done ? 'Consultant payroll finalized' : 'Finalizing Consultant payroll'}
            </DialogTitle>
            <DialogDescription className="text-sm text-muted-foreground">
              {done
                ? 'All employees in this Consultant batch were finalized successfully.'
                : `Finalizing Consultant payroll for ${Math.max(1, Number(totals.employeeCount || payslips.length || 1))} employees...`}
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={showDeleteModal} onOpenChange={(open) => !working && setShowDeleteModal(open)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{finalized ? 'Void Consultant payroll?' : 'Delete Consultant draft?'}</DialogTitle>
            <DialogDescription>
              {finalized
                ? 'This will void the finalized Consultant payroll batch, preserve audit history, and remove it from active payroll lists.'
                : 'This will delete the Consultant draft batch and its draft payslip rows. This action cannot be undone.'}
            </DialogDescription>
          </DialogHeader>
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Bulk send Consultant payslips?</DialogTitle>
            <DialogDescription>
              Send finalized Consultant payslips to all {totals.employeeCount} employees in this batch so they appear in employee My Payslips?
            </DialogDescription>
          </DialogHeader>
          <div className="rounded-xl border border-border/80 bg-background p-3 text-sm text-muted-foreground dark:bg-input/30">
            <p className="font-semibold text-foreground">Batch #{selectedId || '—'}</p>
      ...
      <Dialog open={breakdownRow != null} onOpenChange={(open) => !open && setBreakdownRow(null)}>
        <DialogContent className="max-h-[90vh] overflow-y-auto rounded-2xl border-border bg-card text-card-foreground sm:max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Consultant fixed Basic Pay, allowance, government deduction, and other deduction lines for {breakdownRow?.employee?.name || breakdownRow?.employee_name || 'this employee'}.
            </DialogDescription>
          </DialogHeader>
          {breakdownRow ? (
